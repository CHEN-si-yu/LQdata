"""V79：V76 的**种子复核**单元（seed 3255），其余与 V76 逐字相同。

配方与 V62/V32 逐字相同，唯一差别是特征清单（features_armA_oldcore.json，331 列）。
...
# =============================================================================
VARIANT = '新快照(trainingdata, 599列)上的「旧核」臂：331 列 = 旧337 ∩ 新快照（上游删了 6 列）。其余配方与 V62/V32 逐字相同。'

# ★ 特征清单从文件读（= V32 快照的 columns.features，顺序也照它）⇒ 与 V32 的面板逐列同序。
#   刻意不用 meta['columns']['features']：那是新快照的 653 列，会把对照臂变成处置臂。
FEATURES_OLD = json.loads((Path(__file__).resolve().parent / 'features_armA_oldcore.json').read_text())
