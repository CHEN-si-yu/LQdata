"""V32 样本外评价：逐日 RankIC、可执行 ΣtopN、现金策略与四列打分。
...
from model import (ROOT, RECIPE, Panel, Prices, atomic_json, axis, corr, label_horizon, provenance,
...
def costfree(pred, panel, prices, days, n, horizon=1):
    ...
    for p, d in zip(pred, days):
        ...
        y = panel.Y[f'label_ret_{horizon}d'][d]
...
# 预登记的小排名缓冲，保留头部信号并尝试减少边界抖动换手。
# --- ★ 2026-09-22 RD11（用户指令）：台地 + 止盈止损 ---
...
# --- ★ 2026-09-22 RD11 第二轮：把已验证的 60 日市场门槛接到五日换手台地上 ---
...
# --- ★ 2026-09-22 RD11 第三轮：**台地边界检查**（不是参数加密）---
...
# --- ★ 2026-09-22 RD11 第四轮：**把 exit_rank 的形状补到上界**（形状检查，不是挑参数）---
...
STRATEGIES += [dict(name=f'buffer_r{r}_t{n}', topn=n, exit_rank=r, min_hold=1)
               for n, ranks in [(1, (2, 5, 10)), (5, (10, 20))] for r in ranks]
...
def cash_backtest(pred, panel, px, days, n=5, period=1, exit_rank=None, min_hold=1,
            ...
            if exit_rank is None:
                ...
                for c in list(holdings):
                    ...
                    if c not in head and d - buys[c] >= min_hold:
                        exits.add(c)
                        continue
                    if stop_loss is None and take_profit is None and trailing is None:
                        continue
    ...
    return dict(topn=n, rebalance_days=period, exit_rank=exit_rank, min_hold=min_hold, take_profit=take_profit,
...
def run_pipeline(jobs=8, device='auto'):
    ...
    def sample():
        ...
        row = dict(timestamp=time.time(),
                   ...
                   # ★ 有效占用 = current − file（page cache 可回收，不是 OOM 风险；见 memory_guard.py 注释）
                   effective_gib=(int(Path('/sys/fs/cgroup/memory.current').read_text()) - mem.get('file', 0)) / 1024 ** 3,
    ...
    limit = min(cgroup_memory_limit_gib() or 90., 80_000_000_000 / 1024 ** 3)
    if limit is None:
        ...
        prior_task_peak = max((x['peak_anon_gib'] / max(1, x.get('max_active_tasks', 1)) for x in history), default=8.)
        per_worker = max(9., prior_task_peak * 1.15)
    ...
    for quarter in quarters(days):
        for fold in range(1, 5):
            ...
            if (folder / 'complete.json').exists():
                ...
                if provenance(dict(RECIPE, device=lock['recipe']['device'], quarter=quarter))['run_id'] != done['run_id']:
    ...
    quota_raw = Path('/sys/fs/cgroup/cpu.max').read_text().split()
    quota = len(aff) if quota_raw[0] == 'max' else max(1, int(quota_raw[0]) // int(quota_raw[1]))
    usable_cores = min(len(aff), quota)
    # 单任务核数：本机可用核数的 80% 在并发任务间均分，上限 3 核（再多收益递减、内存更吃紧）。
    cores_per_job = max(1, min(3, int(usable_cores * .95) // max(actual, 1)))
    ...
    with open(logs / 'resource_usage.csv', 'w', newline='') as fh:
        ...
        while True:
            ...
            if limit is not None and (row['anon_gib'] > limit * .90 or row['effective_gib'] > 84_000_000_000 / 1024 ** 3):
    ...
    if not failed and (ROOT / 'model_pred' / 'tables' / 'report.json').exists():
        # 完整资源采样在分析结束后才能落盘，随后刷新报告避免缺失资源表。
        generate_report()
...
def generate_report():
    ...
    title=f'# {RECIPE["name"]} 季度滚动模型：训练、回测与资源报告\n\n'
    ...
    txt=title+intro+'本版本预先登记的实验：'+str(RECIPE.get('time_half_life',0))+'交易日半衰期（0表示不加权）；全部结果均属于研究窗内比较，不能称为独立未见验证。\n\n'+'## 1. 基线与季度结果\n\n'
    ...
    # 切分口径与打分口径都从 recipe 现取：写死会让报告与实际产物不符（静默不一致）。
    scheme=RECIPE.get('split_scheme','history4')
    fmt={'label':RECIPE['label'],'h':label_horizon(RECIPE['label'])}
    if scheme=='tailpool':
        txt+=(f"每季度从 2018 起扩展训练历史，**验证集取自紧贴测试窗的尾部池**"
              f"（{RECIPE.get('valid_pool_days',243)} 个交易日，随机抽 1/4；训练集四折相同），"
              f"{fmt['h']+1} 个交易日隔离。")
    else:
        txt+=(f"每季度从 2018 起扩展训练历史，季度前 {fmt['h']+1} 个交易日隔离；"
              f"{RECIPE['folds']} 个历史验证块轮换并 purge/embargo。")
    if RECIPE.get('epoch_agg','best')=='mean_all':
        txt+=(f"★ **打分口径：全部轮次的预测取平均**（不再选轮）。"
              f"val_wei（100×可执行top5的 {fmt['label']} 代理收益+0.7×逐日Pearson IC）**只用来决定何时早停**，"
              f"测试窗不参与任何选择。")
    else:
        txt+=(f"val_wei=100×可执行top5的 {fmt['label']} 代理收益+0.7×逐日Pearson IC；"
              f"只用于验证选 epoch，测试不参与选择。")
    txt+=(f"调度factor={RECIPE['lr_factor']}/patience={RECIPE['lr_patience']}/cooldown={RECIPE['lr_cooldown']}"
          f"/min_lr={RECIPE['min_lr']:.0e}；{RECIPE['early_stop_patience']} 轮未改善早停，"
          f"种子 {RECIPE['seed']}–{RECIPE['seed']+RECIPE['folds']-1}。\n\n'")
...
def write_daily_picks(variants, panel, px, days, topn=10, recent=10):
    ...
    txt = ('# 最新一日打分与策略建议\n\n'
           ...
           f'**训练标签**：{RECIPE["label"]}；下表按1日交易协议展示：{t1} 买入、{t2} 卖出。\n\n'
...
def main():
    ...
    for fold in fold_ids:
        ...
        for quarter in qs:
            ...
            # ★ 对拍的**目标必须跟着打分的聚合口径走**：V22 起打分是"全部轮次的平均"，
            # 若仍拿 `best.pt` 单个模型去对拍，只会得到"推理重载对拍失败"这个**误导性的**结论
            # （真正的错因是口径不一致）。所以这里按 `recipe['epoch_agg']` 重建同一份聚合打分。
            if args.audit:
                agg = lock['recipe'].get('epoch_agg', 'best')
                epoch_files = sorted((folder / 'epochs').glob('epoch*.pt'))
                ...
                def reload(path):
                    ...
                    net.load_state_dict(snapshot['model'])
                    net.eval()
                    return net

                if agg == 'mean_all' and epoch_files:
                    acc = None
                    for path in epoch_files:
                        q = predict(reload(path), panel, check_days, device_cpu)
                        acc = q if acc is None else acc + q
                    actual = (acc / len(epoch_files)).astype(np.float32)
                    # 逐行独立性用**同一个单模型**自比（比的是 predict() 的实现，不是聚合口径）
                    row_net = reload(epoch_files[0])
                    row_ref = predict(row_net, panel, check_days, device_cpu)
                else:
                    row_net = reload(folder / 'best.pt')
                    row_ref = predict(row_net, panel, check_days, device_cpu)
                    actual = row_ref
                expected = p[check_ix]
                ...
                for j, d in enumerate(check_days):
                    mask = panel.mask(d)
                    with torch.no_grad():
                        one = row_net(torch.from_numpy(panel.X[d, mask][:10])).squeeze(-1).numpy()
                    if not np.allclose(one, row_ref[j, mask][:10], atol=2e-5, rtol=2e-5):
    ...
    # 门槛指数只依赖 panel/prices，算一次给全部门槛格共用（带因果性自检）。
        ...
        if px:
            ...
            for spec in STRATEGIES:
                ...
                stat, curve, trades = cash_backtest(
                    ...
                    stop_loss=spec.get('stop_loss'), take_profit=spec.get('take_profit'),
    ...
    if px:
        ...
        def metrics_at(pred, days_, horizon):
