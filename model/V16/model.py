"""V16 简单基线：MLP-4linear + WPCC。

仅依赖本文件、analysis.py 与 trainingdata；运行 python model.py --help。

职责边界：本文件只管「单折训练」——给它一个季度和一个折号，它训练、验证选轮、
打分落盘后退出，不知道也不关心别的折在做什么。跨折的并发调度、内存闸门、
回测与报告都在 analysis.py；统一入口是 train.sh。
"""
...
RECIPE = dict(
    ...
    valid_money=100000,
    valid_topn=5,
    valid_metric='100 * executable_top5_5d_mean_return + 0.7 * Pearson_IC',
...
class Panel:
    ...
    def __init__(self, features='all', load_x=True):
        ...
        self.features = (self.meta['columns']['features'] if features == 'all'
...
def validation(model, panel, prices, days, device, label):
    ...
    return dict(val_wei=100 * ret + .7 * ic, val_return_proxy=ret, Pearson_IC=ic, RankIC=float(np.mean(ranks)))
...
def train_fold(fold, device='auto', panel=None, prices=None, quarter=None):
    ...
    split = splits(panel.days, 5, recipe['folds'], quarter)[fold - 1]
    ...
    for epoch in range(start, recipe['max_epochs']):
        ...
        metrics = validation(model, panel, prices, split['valid'], dev, recipe['label'])
    ...
    result = dict(run_id=rid, quarter=quarter, fold=fold, best_epoch=ck['epoch'] + 1, epochs=len(history),
...
def main():
    parser = ArgumentParser(description='V16：季度滚动四折 MLP 基线，单折训练（断点续跑）')
